...
class FrozenCLIPEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        local_openai_clip_dir = os.environ.get("DUALA_OPENAI_CLIP_DIR", "/data/data/gjt/openai-clip-vit-large-patch14")
        # Many OpenCLIP text towers still use the CLIP-L tokenizer/text config; in offline mode
        # we fall back to the shared local CLIP-L cache for both openai and laion aliases.
        if version == "openai/clip-vit-large-patch14" and os.path.isdir(local_openai_clip_dir):
            resolved_version = local_openai_clip_dir
        elif isinstance(version, str) and version.startswith("laion2b_") and os.path.isdir(local_openai_clip_dir):
            resolved_version = local_openai_clip_dir
        self.tokenizer = CLIPTokenizer.from_pretrained(resolved_version, local_files_only=True)
        self.transformer = CLIPTextModel.from_pretrained(resolved_version, local_files_only=True)
        self.device = device
        self.max_length = max_length
        if freeze:
            self.freeze()
...
class FrozenOpenCLIPEmbedder2(AbstractEmbModel):
    """
    Uses the OpenCLIP transformer encoder for text while supporting offline local
    cache fallbacks for both OpenAI CLIP-L and laion aliases.
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert layer in self.LAYERS
        local_openai_clip_dir = os.environ.get(
            "DUALA_OPENAI_CLIP_DIR", "/data/data/gjt/openai-clip-vit-large-patch14"
        )
        resolved_version = version
        resolved_cache_dir = cache_dir
        if version == "openai/clip-vit-large-patch14" and os.path.isdir(local_openai_clip_dir):
            resolved_version = "openai"
            resolved_cache_dir = local_openai_clip_dir

        model, _, _ = open_clip.create_model_and_transforms(
            arch,
            device=torch.device("cpu"),
            pretrained=resolved_version,
    ...
    def encode_with_transformer(self, text):
        ...
        if getattr(self.model, "text_projection", None) is not None:
