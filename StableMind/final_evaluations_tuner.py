...
parser.add_argument(
    "--model_name", type=str, default="final_subj01_pretrained_1sess_24bs_AugBlur_P1.0_W0.3",
    help="name of model, used for ckpt saving and wandb logging (if enabled)",
)
# parser.add_argument(
#     "--all_recons_path", type=str, default="/ssd/gjt/MindEyeV2-results/train_logs/evals/final_subj01_pretrained_1sess_24bs/final_subj01_pretrained_1sess_24bs_all_recons.pt",
#     help="Path to where all_recons.pt is stored",
# )

parser.add_argument(
    "--enhanced_flag", type=int, default=1,
    help="",
)

parser.add_argument("--enable_learnnorm", type=int, default=0, help="learnable normalization")
...
parser.add_argument(
    "--cache_dir", type=str, default="/data/data/shumeng/hub//",
...
parser.add_argument("--ap_mode",type=int,default=0, help="0=pred cls, 1=real cls")
...
all_images = torch.load(f"{args.data_path}/evals/all_images.pt")
all_captions = torch.load(f"{args.data_path}/evals/all_captions.pt")

# print("all_recons_path:", args.all_recons_path)
# all_recons = torch.load(args.all_recons_path)

# print("all_recons_path:", args.all_recons_path)

if args.enhanced_flag == 0:
    all_recons_path = f"{args.ckpt_root}/evals/{args.model_name}/all_recons.pt"
    model_name_plus_suffix = f"{args.model_name}_all_recons"
else:
    if args.ap_mode == 0:
        all_recons_path = f"{args.ckpt_root}/evals/{args.model_name}/all_enhancedrecons.pt"
        model_name_plus_suffix = f"{args.model_name}_all_enhancedrecons"
    else:
        all_recons_path = f"{args.ckpt_root}/evals/{args.model_name}/realCLS_all_enhancedrecons.pt"
        model_name_plus_suffix = f"{args.model_name}_realCLS_all_enhancedrecons"
all_recons = torch.load(all_recons_path)
# Residual submodule
all_clipvoxels = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/all_clipvoxels.pt")
# Low-level submodule
all_blurryrecons = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/all_blurryrecons.pt")
# GIT predicted captions
all_predcaptions = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/all_predcaptions.pt")
...
# from inpainting_images import FoveaBlur, FoveaBlurTorch, FoveaBlurTorch_Fast
# img_augment = AugmentationSequential(
#             FoveaBlurTorch(h=224, w=224, blur_kernel_size=51, curve_type='exp', system_g=3),
#             same_on_batch=False,
#             data_keys=["input"],
#         )
# all_images = img_augment(all_images)
...
brain_region_masks = {}
with h5py.File(f"{args.data_path}/brain_region_masks.hdf5", "r") as file:
...
subject_masks = brain_region_masks[f"subj0{args.subj}"]
...
GNet = GNet8_Encoder(device=device,subject=args.subj,model_path=f"{args.data_path}/gnet_multisubject.pt")
...
os.makedirs(f'{args.ckpt_root}/tables/',exist_ok=True)
df["Value"].to_csv(f'{args.ckpt_root}/tables/{model_name_plus_suffix}.csv', sep='\t', index=False)

#
# # # Captions evaluation
# all_git_generated_captions = torch.load(f"{args.data_path}/evals/all_git_generated_captions.pt")
#
# # ## Meteor
# # meteor_path = "/ssd/lsm/hub/modules/evaluate_modules/metrics/evaluate-metric--meteor/90426c9b4a2cf0bf10c6718aa8de221c7a50f46a694f4cc37bb3ee12fc9aa8c9"
# # meteor = evaluate.load(metric_dir)
# # meteor = evaluate.load('meteor',
# #                        cache_dir="/ssd/lsm/hub/modules/evaluate_modules"
# #                        )
# # meteor = evaluate.load("/ssd/lsm/hub/modules/evaluate_modules/metrics/evaluate-metric--meteor")
# meteor = evaluate.load('/ssd/gjt/evaluate_matrics/evaluate/metrics/meteor/meteor.py')
...
# rouge = evaluate.load('/ssd/gjt/evaluate_matrics/evaluate/metrics/rouge/rouge.py')
...
# sentence_model = SentenceTransformer('/ssd/lsm/hub/models--sentence-transformers--all-MiniLM-L6-v2/snapshots/c9745ed1d9f207416be6d2e6f8de32d1f16199bf')
#
#
# with torch.no_grad():
...
#
# # #### CLIP-B
# path_clip_vit_base_patch32 = "/ssd/lsm/hub/models--openai--clip-vit-base-patch32/snapshots/3d74acf9a28c67741b2f4f2ea7635f0aaf6f0268"
# model_clip = CLIPModel.from_pretrained(path_clip_vit_base_patch32)
# processor_clip = AutoProcessor.from_pretrained(path_clip_vit_base_patch32)
# tokenizer =  AutoTokenizer.from_pretrained(path_clip_vit_base_patch32)
...
#
# # #### CLIP-L
# path_clip_vit_large_patch14 = "/ssd/lsm/hub/models--openai--clip-vit-large-patch14/snapshots/32bd64288804d66eefd0ccbe215aa642df71cc41/"
# model_clip = CLIPModel.from_pretrained(path_clip_vit_large_patch14)
# processor_clip = AutoProcessor.from_pretrained(path_clip_vit_large_patch14)
# tokenizer =  AutoTokenizer.from_pretrained(path_clip_vit_large_patch14)
...
# df.to_csv(f'{args.ckpt_root}/tables/{model_name_plus_suffix}_caption_metrics.csv', sep='\t', index=False)
#
# #
# # # # UMAP
# # import umap
...
# #
# # print(args.all_recons_path)
# # all_recons = torch.load(args.all_recons_path)
# # all_backbones = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/{args.model_name}_all_backbones.pt")
# # all_prior_out = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/{args.model_name}_all_prior_out.pt")
# # all_clipvoxels = torch.load(f"{args.ckpt_root}/evals/{args.model_name}/{args.model_name}_all_clipvoxels.pt")
# #
# # # Image feature
...
# # plt.savefig(f'{args.model_name}_umap_retrieval.png')
# # plt.show()
# #
# #
# # # Prior_Out
...
# # plt.savefig(f'{args.model_name}_umap_prior.png')
# # plt.show()
# #
# #
# # # Backbones
...
# # plt.savefig(f'{args.model_name}_umap_backbone.png')
# # plt.show()
